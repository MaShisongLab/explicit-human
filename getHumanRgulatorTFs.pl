use strict;
my $pvalue_cutoff = 1E-12;  			# pValue cut-off for TF-target gene pair.  
my $qvalue_cutoff = 1E-5;  			# qvalue cut-off for a TF to be considered as predictor TF for a module.
my $total_gene = 23346 + 1613; 			# non-TF genes plus TF genes.
...
my $target_gene_file = "./data/Hs.target.genes.txt";
my $gene_symbl_file = "./data/Hs.gene.symbls.txt";
my $edge_file_1 = "./data/Hs.SigEdges.part1.txt"; 		# with singinificant interacting TF-target gene pair.
my $edge_file_2 = "./data/Hs.SigEdges.part2.txt"; 		# with singinificant interacting TF-target gene pair.
...
unless( -e $edge_file_1 ){
	print "The file 'Hs.SigEdges.part1.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getHumanRegulatorTFs.pl' should not be changed.\nExit in 5 second.\n";
	sleep 5;
	exit 1;
}

unless( -e $edge_file_2 ){
	print "The file 'Hs.SigEdges.part2.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getHumanRegulatorTFs.pl' should not be changed.\nExit in 5 second.\n";
	sleep 5;
	exit 1;
}

unless( -e $gene_symbl_file ){
	print "The file 'Hs.gene.symbls.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getHumanRegulatorTFs.pl' should not be changed.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}

unless( -e $target_gene_file ){
	print "Target gene file 'Hs.target.genes.txt' not found. It should be saved in the 'data' sub-directory. And the location of the script 'getHumanRegulatorTFs.pl' should not be changed.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}

unless( -e $cluster_file ){
	print "The file 'modules_to_analyze.txt' not found. It should be saved in the same directory as the script 'getHumanRegulatorTFs.pl'.\nExit in 5 seconds.\n";
	sleep 5;
	exit 1;
}
...
while ( $l = <IN>){
	...
	if ( length( $t[0] ) > 6 & $t[3] <= $pvalue_cutoff ){
		$e{$t[0]} .= "$t[1] $coef $t[3]\t";
		$tfc{$t[1]} ++;
	}
}
...
while ( $l = <IN>){
	...
	$t[0] =~ s/^\s+|\s+$//;
	$t[0] = uc ($t[0]);
	if ( $seen{$t[0]}){				# only consider target genes listed in ./data/Hs.target.genes.txt 
